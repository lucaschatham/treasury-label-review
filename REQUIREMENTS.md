...
| 2 | Dave: STONE'S THROW and Stone's Throw mean the same brand. | Verify this exact capitalization example and a genuinely different brand through image review. Do not let normalization conceal an actual difference. | IMPLEMENTED: normalization and independent layout regressions pass; final deployed case-difference smoke check pending. |
...
| 7 | Sarah: clean, obvious interface for mixed technical comfort. Evaluation: UX and error handling. | Verify sample, upload, entering details, finding a discrepancy, inspecting artwork, batch review, and recovering from invalid/unreadable input. Check narrow layout and keyboard operation as practical usability evidence. | IMPLEMENTED: selected interface preserved, original and crop evidence shown; final preview keyboard/narrow/error smoke checks pending. |
