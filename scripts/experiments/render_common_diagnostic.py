"""R-016 preparation only. No inference. Refuses existing output."""
import sys,json,hashlib,subprocess,io
from pathlib import Path
import numpy as np
from PIL import Image,ImageDraw,ImageFont
from fontTools.ttLib import TTFont
from render_controls import validate_pair,validate_matrix,write_new,common_height,png_options
...
out=ROOT/'evidence/render-diagnostic-r016-corrected';out.mkdir(exist_ok=False)
...
def render(path,size,word,renderer):
 ...
 data=subprocess.check_output(['magick','-size','1200x160','xc:white','-fill','black','-font',str(path),'-pointsize',str(size),'-annotate','+20+100',word,*png_options(),'png:-']);return Image.open(io.BytesIO(data)).convert('RGB')
...
for family in families:
 for weight in ['REGULAR','BOLD']:
  ...
  fonts[family,weight]=(path,meta)
  for renderer in ['pillow','magick']:
   mapping={}
   for size in range(7,65):
    b=box(render(path,size,'H',renderer),128);mapping.setdefault(b[3]-b[1],size)
   sizes[family,weight,renderer]=mapping
 for target in [12,24]:
  chosen[family,target]=common_height([set(sizes[family,w,r]) for w in ['REGULAR','BOLD'] for r in ['pillow','magick']],target)
write_new(out/'heights.json',json.dumps({f'{f}-{t}':h for (f,t),h in chosen.items()},indent=2))
for family in families:
 for weight in ['REGULAR','BOLD']:
  path,meta=fonts[family,weight]
  for target in [12,24]:
   pair=[]
   for renderer in ['pillow','magick']:
    measured=chosen[family,target]
    size=sizes[family,weight,renderer][measured]
    ...
    r=dict(id=key,family=family,expected=weight,renderer=renderer,height=measured,targetHeight=target,size=size,font=sha(path),originalFont=meta,clipped=bool(clipped),boxes=boxes,tensorSha256=hashlib.sha256(tensor.tobytes()).hexdigest());pair.append(r);result.append(r)
   validate_pair(*pair)
