...
def main():
 ...
 assert sha(a.source/'head.pt')==source['headSha256']
 ...
 result=dict(experiment=source.get('experiment','R-027'),decision='ADVANCE',scope='Exposed calibration only',modelFile='model.onnx',modelSha256=sha(a.output/'model.onnx'),sourceResultSha256=sha(a.source/'result.json'),sourceHeadSha256=sha(a.source/'head.pt'),canonicalPolarity=True,polarityCodeSha256=sha(Path(__file__).with_name('polarity.py')),inputCodeSha256=sha(Path(__file__).with_name('fontdna_input.py')),runnerCodeSha256=sha(__file__),**margin)
