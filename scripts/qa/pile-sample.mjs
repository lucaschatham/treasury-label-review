...
await page.locator('#status').filter({hasText:'Finished:'}).waitFor({state:'attached',timeout:120000});
...
await row.click();
await page.getByRole('dialog').getByLabel('Note (optional)',{exact:true}).fill('Verified sample');
await page.getByRole('dialog').getByRole('button',{name:'✓ Approve',exact:true}).click();
await page.locator('.large.passed').click();
assert.match(await row.textContent(),/You/);
