...
const [url, fixtureDir, nodeResultPath, output] = process.argv.slice(2);
const manifest = JSON.parse(await readFile(`${fixtureDir}/manifest.json`, 'utf8'));
const nodeRows = Object.fromEntries(JSON.parse(await readFile(nodeResultPath, 'utf8')).rows.map(r => [r.id, r]));
...
try {
  await page.goto(url);
  await page.locator('#images').setInputFiles(manifest.rows.map(r => `${fixtureDir}/${r.file}`));
  ...
  const rows = await page.evaluate(() => [...document.querySelectorAll('.triage-row')].map(row => ({
    ...
    appearance: JSON.parse(row.dataset.appearance || 'null'), timings: JSON.parse(row.dataset.timings || 'null') })));
  const byFile = {};
  ...
  const comparisons = rows.map(row => {
    ...
    return { file: item?.file || row.name, id: item?.id, expected: item?.expected, browserVerdict, nodeVerdict: node?.verdict, browserRatio: row.appearance?.ratio ?? null, nodeRatio: node?.ratio ?? null,
  ...
  const summary = { url, fixtures: manifest.rows.length, rowsRendered: rows.length, status: status.trim(), batchSeconds: (finished - clicked) / 1000,
    verdictAgreement: `${comparisons.filter(c => c.agrees).length}/${comparisons.filter(c => c.nodeVerdict).length}`,
