...
The user selected Path A. The local measurement is integrated (`src/weight.js`, `src/appearance.js`), the cloud proxy and Worker are removed, and the candidate passed independent qualification on the eight reserved families (R-040: 39/40 bold, 0/40 regular, 32/32 bold-body controls Review) with identical browser verdicts on all 112 fixtures and uncached sample timing of 1.35–1.78 s. See the "Path A execution" section of REQUIREMENTS.md. Deployment is the remaining step and was not performed from the investigation environment.
