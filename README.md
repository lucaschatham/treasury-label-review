...
- **Deterministic comparisons:** case/apostrophe/punctuation normalization for text fields, contextual ABV and proof consistency (the OCR misreading "Ale./Vol." is accepted as alcohol context; a bare percentage is not), numeric volume conversion, complete warning wording with internal punctuation. An unread final period is the one tolerated OCR omission: the finding still matches but names the unread period for the reviewer. Warning body capitalization and whitespace are normalized; the heading must remain uppercase. This is an OCR wording check, not a byte-for-byte artwork comparison.
...
- `python3 scripts/experiments/render_full_labels.py` then `node scripts/experiments/run_weight_contrast.mjs`: full-label weight-contrast fixtures from pinned Google Fonts sources through the application's OCR configuration and measurement in Node (R-039 to R-042). `render_defect_corpus.py` renders wrong-value, altered-wording, title-case, missing-warning and brand-case variants for the browser batch check (`scripts/qa/batch-fixtures.mjs`). Requires Python with Pillow, numpy and fontTools.
- The retired cloud-vision harnesses and the FontDNA/MobileNet experiment scripts (R-001 to R-036) were removed from the tree on September 24; they depended on local worktrees and services that are not part of this repository. They remain in git history at commit `8d0571a`, and their evidence stays under `evidence/`.
